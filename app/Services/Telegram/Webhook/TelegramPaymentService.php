...
class TelegramPaymentService
{
    ...
    public function handlePreCheckoutQuery(TelegramPreCheckoutData $dto): void
    {
        ...
        if (! $payload) {
            Log::channel('telegram')->warning('telegram.pre_checkout.invalid_payload', ['query_id' => $dto->queryId]);
            $response = $this->botService->answerPreCheckoutQuery($dto->queryId, false, 'Invalid payload.');
            Log::channel('telegram')->info('telegram.pre_checkout.answer', ['query_id' => $dto->queryId, 'ok' => (bool) ($response['ok'] ?? false)]);
        ...
        if (! $subscription) {
            Log::channel('telegram')->warning('telegram.pre_checkout.subscription_not_found', ['query_id' => $dto->queryId, 'subscription_id' => $payload['subscription_id']]);
        ...
        if (! $this->isSubscriptionAvailable($subscription)) {
            Log::channel('telegram')->warning('telegram.pre_checkout.subscription_unavailable', ['query_id' => $dto->queryId, 'subscription_id' => (int) $subscription->id]);
        ...
        if ($dto->totalAmount !== $expectedAmount) {
            Log::channel('telegram')->warning('telegram.pre_checkout.amount_mismatch', ['query_id' => $dto->queryId, 'expected_amount' => $expectedAmount, 'received_amount' => $dto->totalAmount]);
    ...
    public function handleSuccessfulPayment(TelegramSuccessfulPaymentData $dto): void
    {
        if (Payment::query()->where('telegram_payment_charge_id', $dto->telegramPaymentChargeId)->exists()) {
            Log::channel('telegram')->info('telegram.payment.duplicate', ['telegram_payment_charge_id' => $dto->telegramPaymentChargeId]);
        ...
        if (! $payload) {
            Log::channel('telegram')->warning('telegram.payment.invalid_payload', ['telegram_payment_charge_id' => $dto->telegramPaymentChargeId]);
        ...
        if (! $subscription || ! $this->isSubscriptionAvailable($subscription)) {
            Log::channel('telegram')->warning('telegram.payment.subscription_unavailable', ['telegram_payment_charge_id' => $dto->telegramPaymentChargeId, 'subscription_id' => $subscriptionId]);
        ...
        $this->subscriptionLifecycle->syncActivityQueue($customerId, (int) $subscription->activity_id, $startDate);
        ...
        $this->botService->sendMessage(
            ...
            ['parse_mode' => 'HTML', 'reply_markup' => $this->miniAppInlineKeyboard($effectiveStatus)]
        );

        Log::channel('telegram')->info('telegram.payment.success_notification_queued', ['chat_id' => $dto->chatId, 'telegram_payment_charge_id' => $dto->telegramPaymentChargeId]);
    }

    public function makeInvoiceForLinkedUser(int $telegramUserId, int $subscriptionId): array
    {
        Log::channel('telegram')->info('telegram.invoice.prepare.start', ['telegram_user_id' => $telegramUserId, 'subscription_id' => $subscriptionId]);
        ...
        if (! $link || ! $link->customer) {
            Log::channel('telegram')->warning('telegram.invoice.prepare.not_linked', ['telegram_user_id' => $telegramUserId]);
        ...
        if (! $subscription) {
            Log::channel('telegram')->warning('telegram.invoice.prepare.subscription_not_found', ['telegram_user_id' => $telegramUserId, 'subscription_id' => $subscriptionId]);

            return ['ok' => false, 'status' => 404, 'message' => 'Subscription not found.'];
        }

        if (! $this->isSubscriptionAvailable($subscription)) {
            Log::channel('telegram')->warning('telegram.invoice.prepare.subscription_unavailable', ['telegram_user_id' => $telegramUserId, 'subscription_id' => $subscriptionId]);
        ...
        if ($activeSamePlan && $activeSamePlan->remaining_visits === null) {
            Log::channel('telegram')->warning('telegram.invoice.prepare.duplicate_active_subscription', ['telegram_user_id' => $telegramUserId, 'customer_id' => (int) $link->customer->id, 'subscription_id' => (int) $subscription->id]);
        ...
        if ($activeSamePlan && (int) $activeSamePlan->remaining_visits > 1) {
            Log::channel('telegram')->warning('telegram.invoice.prepare.repurchase_too_early', ['telegram_user_id' => $telegramUserId, 'customer_id' => (int) $link->customer->id, 'remaining_visits' => (int) $activeSamePlan->remaining_visits]);

            return ['ok' => false, 'status' => 422, 'message' => 'You can buy this plan when only 1 visit is left.'];
        }

        $providerToken = (string) config('services.telegram.provider_token');
        if ($providerToken === '') {
            Log::channel('telegram')->warning('telegram.invoice.prepare.missing_provider_token', ['telegram_user_id' => $telegramUserId]);
        ...
        $payload = sprintf('cs_%d_%d_%s', (int) $link->customer->id, (int) $subscription->id, $nonce);
        ...
        $response = $this->botService->sendInvoice((int) $telegramUserId, [
            ...
            'currency' => 'UZS',
            'prices' => json_encode([['label' => $title, 'amount' => $this->toMinorUnits($finalPrice)]], JSON_UNESCAPED_UNICODE),
            'start_parameter' => 'subscription_' . $subscription->id,
        ]);

        Log::channel('telegram')->info('telegram.invoice.prepare.telegram_response', ['telegram_user_id' => $telegramUserId, 'subscription_id' => $subscriptionId, 'ok' => (bool) ($response['ok'] ?? false)]);

        if (($response['ok'] ?? false) !== true) {
            return ['ok' => false, 'status' => 422, 'message' => (string) ($response['description'] ?? 'Failed to send invoice.')];
        }

        return ['ok' => true, 'status' => 200, 'message' => 'Invoice sent to Telegram chat.'];
    }

    private function parseInvoicePayload(string $payload): ?array
    {
        if (preg_match('/^cs_(\d+)_(\d+)_([a-z0-9]+)$/i', $payload, $m) === 1) {
            return ['customer_id' => (int) $m[1], 'subscription_id' => (int) $m[2]];
        }
        ...
        if (is_array($decoded)) {
            ...
            if ($customerId > 0 && $subscriptionId > 0) {
                return ['customer_id' => $customerId, 'subscription_id' => $subscriptionId];
            }
        }
    ...
    private function miniAppInlineKeyboard(string $status): string
    {
        ...
        return json_encode([
            'inline_keyboard' => [
                [['text' => $buttonText, 'web_app' => ['url' => $this->miniAppUrl()]]],
            ],
