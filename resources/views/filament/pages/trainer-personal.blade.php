<x-filament::page>
    ...
    <div class="manager-personal space-y-6">
        ...
        <div class="tp-top-row">
            <x-filament::section heading="Trainer Card" class="manager-personal-section manager-personal-section--card tp-top-row__card">
        ...
        <div class="tp-stats-bar">
            @foreach ($this->stats as $stat)
                <div class="tp-stat-card">
                    <div class="tp-stat-value">{{ $stat['value'] }}<span class="tp-stat-suffix">{{ $stat['suffix'] }}</span></div>
        ...
        <div class="grid grid-cols-1 gap-8">
            <x-filament::section heading="This Week Schedule" class="manager-personal-section">
                @if (count($this->weekScheduleRows))
                    <div class="overflow-x-auto manager-personal-table-wrap">
                        <table class="manager-personal-table">
                            ...
                            <tbody>
                                @foreach ($this->weekScheduleRows as $row)
                                    <tr class="{{ $row['is_today'] ? 'tp-row--today' : '' }}">
                                        <td>{{ $row['day'] }}</td>
                                        <td>{{ $row['activity'] }}</td>
                                        <td>{{ $row['hall'] }}</td>
                                        <td>{{ $row['time'] }}</td>
                ...
                @else
                    <div class="manager-personal-empty">No schedule assigned.</div>
                @endif
            </x-filament::section>
